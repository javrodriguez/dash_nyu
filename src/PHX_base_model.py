...
class ODENet(nn.Module):
    ...
    def __init__(self, device, ndim, explicit_time=False, neurons=100, log_scale = "linear", init_bias_y = 0, init_sparsity = 0.95):
        ...
        for n in self.net_sums.modules():
            if isinstance(n, nn.Linear):
                nn.init.sparse_(n.weight,  sparsity=self.init_sparsity, std = 0.05) 
    ...
    def save(self, fp):
        ...
        base_path, ext = os.path.splitext(fp)
        alpha_comb_sums_path = base_path + '_alpha_comb_sums' + ext
        alpha_comb_prods_path = base_path + '_alpha_comb_prods' + ext
        gene_mult_path = base_path + '_gene_multipliers' + ext
        
        prod_path = base_path + '_prods' + ext
        sum_path = base_path + '_sums' + ext
    ...
    def inherit_params(self, fp):
        ...
        with torch.no_grad():
            X = torch.load(prod_path, weights_only=False)
            ...
            X = torch.load(sum_path, weights_only=False)
    ...
    def load_model(self, fp):
        ...
        self.net_prods = torch.load(prod_path, weights_only=False)
        self.net_sums = torch.load(sum_path, weights_only=False)
        self.gene_multipliers = torch.load(gene_mult_path, weights_only=False)
        self.net_alpha_combine_sums = torch.load(alpha_comb_sums_path, weights_only=False)
        self.net_alpha_combine_prods = torch.load(alpha_comb_prods_path, weights_only=False)
