...
def _convert_arguments(settings):
    ...
    converted_settings['normalize_data'] = settings.getboolean('normalize_data', fallback=False)
    ...
    converted_settings['pruning'] = settings.getboolean('pruning', fallback=False)
    converted_settings['lr_range_test'] = False 
    converted_settings['scale_expression'] = settings.getint('scale_expression', fallback=1)
    converted_settings['log_scale'] = settings.get('log_scale', fallback='linear')
    converted_settings['init_bias_y'] = settings.getfloat('init_bias_y', fallback=0)
    converted_settings['init_sparsity'] = settings.getfloat('init_sparsity', fallback=0.95)
    converted_settings['compute_prior_loss'] = settings.getboolean('compute_prior_loss', fallback=False)
    converted_settings['seed'] = settings.getint('seed', fallback=42)
